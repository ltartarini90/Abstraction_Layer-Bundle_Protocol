...
dtn_bundle_spec_t al_dtn_bundle_spec(al_bp_bundle_spec_t bundle_spec)
{
	...
	memset(&dtn_bundle_spec, 0, sizeof(dtn_bundle_spec_t));
...
al_bp_bundle_spec_t dtn_al_bundle_spec(dtn_bundle_spec_t bundle_spec)
{
	...
	memset(&bp_bundle_spec, 0, sizeof(al_bp_bundle_spec_t));        
	...
	bp_bundle_spec.delivery_regid = dtn_al_reg_id(bundle_spec.delivery_regid);
        ...
        if(bp_bundle_spec.blocks.blocks_len == 0 || bundle_spec.blocks.blocks_val == NULL)
        {
            bp_bundle_spec.blocks.blocks_len = 0;
        ...
        else
        {
            ...
            {
                block = bundle_spec.blocks.blocks_val[i];
                bp_bundle_spec.blocks.blocks_val[i].type = block.type;
                bp_bundle_spec.blocks.blocks_val[i].flags = block.flags;
                bp_bundle_spec.blocks.blocks_val[i].data.data_len = block.data.data_len;
                if(block.data.data_len == 0)
                ...
                {                    
                    ...
                    memcpy(bp_bundle_spec.blocks.blocks_val[i].data.data_val, 
                            block.data.data_val, (block.data.data_len) + 1);
                    bp_bundle_spec.blocks.blocks_val[i].data.data_val = (char*) block.data.data_val;
                }
            }
        }
	if (bundle_spec.metadata.metadata_len == 0 || bundle_spec.metadata.metadata_val == NULL)
        {
            bp_bundle_spec.metadata.metadata_len = 0;
	...
	else
	{
            ...
            {
                block = bundle_spec.metadata.metadata_val[i];
                bp_bundle_spec.metadata.metadata_val[i].type = block.type;
                bp_bundle_spec.metadata.metadata_val[i].flags = block.flags;
                bp_bundle_spec.metadata.metadata_val[i].data.data_len = block.data.data_len;
                if(block.data.data_len == 0)
                ...
                {                    
                    ...
                    memcpy(bp_bundle_spec.metadata.metadata_val[i].data.data_val, 
                            block.data.data_val, (block.data.data_len) + 1);
                    bp_bundle_spec.metadata.metadata_val[i].data.data_val = (char*) block.data.data_val;
                }
            }
	}
        /**********************************************************************/
