...
void bp_dtn_free_extension_blocks(al_bp_bundle_spec_t* spec)
{
        int i;
	/*dtn_bundle_spec_t dtn_spec = al_dtn_bundle_spec(*spec);
        for ( i=0; i<dtn_spec.blocks.blocks_len; i++ ) {
            printf("Freeing extension block [%d].data at 0x%08X\n",
                               i, dtn_spec.blocks.blocks_val[i].data.data_val);
            free(dtn_spec.blocks.blocks_val[i].data.data_val);
        }
        dtn_spec.blocks.blocks_val = NULL;
        dtn_spec.blocks.blocks_len = 0;        
	*spec = dtn_al_bundle_spec(dtn_spec);*/
...
void bp_dtn_free_metadata_blocks(al_bp_bundle_spec_t* spec)
{
        int i;
	/*dtn_bundle_spec_t dtn_spec = al_dtn_bundle_spec(*spec);
        for ( i=0; i<dtn_spec.metadata.metadata_len; i++ ) {
            printf("Freeing metadata block [%d].data at 0x%08X\n",
                               i, dtn_spec.metadata.metadata_val[i].data.data_val);
            free(dtn_spec.metadata.metadata_val[i].data.data_val);
        }
        dtn_spec.metadata.metadata_val = NULL;
        dtn_spec.metadata.metadata_len = 0;        
	*spec = dtn_al_bundle_spec(dtn_spec);*/
